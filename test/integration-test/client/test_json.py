...
class ROSBridgeJSONTest:
    ...
    def on_message(self, ws, message):
        ...
        try:
            ...
            if msg.get("topic") == "/chatter":
                ...
                print(
                    f"Received chatter message #{self.message_count['chatter']}: {msg['msg']['data']} [{'✓' if is_valid else '✗'}]"
                )
                ...
                print(
                    f"Received pointcloud message #{self.message_count['pointcloud']}: {num_points} points [{'✓' if is_valid else '✗'}]"
                )
            ...
            self.results["tests"].append(
                {"test": "message_parse", "status": "error", "error": str(e)}
            )
            self.results["overall_status"] = "FAILED"
            if self.ws:
                self.ws.close()
    ...
    def on_error(self, ws, error):
        ...
        self.results["tests"].append({"test": "connection", "status": "error", "error": str(error)})
