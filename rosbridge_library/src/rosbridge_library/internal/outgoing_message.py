...
class OutgoingMessage:
    ...
    def get_bson_values(self):
        ...
        logger.error(f"[BSON_DEBUG] get_bson_values ENTRY for {msg_type} (size: {msg_size} bytes)")
        ...
        if type_check_elapsed > 0.001:  # Log slow type checks
            logger.error(f"[BSON_DEBUG] str(type()) operation took {type_check_elapsed*1000:.3f}ms")
        ...
        if sizing_elapsed > 0.001:  # Log slow sizing operations
            logger.error(f"[BSON_DEBUG] message sizing operation took {sizing_elapsed*1000:.3f}ms")
        
        # Log before calling extract_bson_values
        extract_start = time.time()
        pre_extract_elapsed = extract_start - entry_timestamp
        logger.error(f"[BSON_DEBUG] About to call extract_bson_values() (pre-processing: {pre_extract_elapsed*1000:.3f}ms)")
        
        result = extract_bson_values(self._message)
        
        extract_elapsed = time.time() - extract_start
        logger.error(f"[BSON_DEBUG] extract_bson_values() completed in {extract_elapsed*1000:.3f}ms")
        ...
        if isinstance(result, dict):
            ...
            logger.error(f"[BSON_DEBUG] Result is dict with {len(result)} keys")
            ...
            logger.error(f"[BSON_DEBUG] Estimated result size: {approx_size} bytes (optimized calculation)")
            if large_binary_fields:
                logger.error(f"[BSON_DEBUG] Large binary fields: {large_binary_fields}")
            ...
            if binary_fields:
                logger.error(f"[BSON_DEBUG] Binary fields found: {binary_fields}")
            if binary_detection_elapsed > 0.001:
                logger.error(f"[BSON_DEBUG] Binary field detection took {binary_detection_elapsed*1000:.3f}ms")
        
        result_processing_elapsed = time.time() - result_processing_start
        if result_processing_elapsed > 0.001:
            logger.error(f"[BSON_DEBUG] Result processing took {result_processing_elapsed*1000:.3f}ms")
        
        elapsed = time.time() - start_time
        logger.error(f"[BSON_DEBUG] get_bson_values EXIT for {msg_type} took {elapsed*1000:.3f}ms")
        ...
        if elapsed > 0.1:  # More than 100ms
            ...
            logger.error(f"[BSON_DEBUG] TIME BREAKDOWN: extract={extract_elapsed*1000:.1f}ms ({extract_percentage:.1f}%), other={other_time*1000:.1f}ms ({100-extract_percentage:.1f}%)")
